define({
 ...
 "dbName": "flude",
  ...
  "queries": {
    ...
    "pubs_reformat": "SELECT CONCAT('',publications.category),	CONCAT('',publications.title),  CONCAT('',publications.description),  CONCAT('',publications.publication_date), '',  CONCAT('',publications.source),  CONCAT('',publications.author_name),  '',  CONCAT('',publications.language),   '',  '',  '',  CONCAT('',publications.document_size),  CONCAT('',publications.document_attachment_name),   CONCAT('',publications.document_attachment_title),   '','' FROM public.publications",
